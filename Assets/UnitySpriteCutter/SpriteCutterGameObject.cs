...
namespace UnitySpriteCutter {
	...
	internal class SpriteCutterGameObject {
		...
		public static SpriteCutterGameObject CreateAs( GameObject origin ) {
			...
			// check if there is a parent, if so, use the parent's scale - RYAN CODE
			if(origin.transform.parent != null)
				result.gameObject.transform.localScale = origin.transform.parent.localScale;
			else 
				result.gameObject.transform.localScale = origin.transform.localScale;
		...
		void CopyGameObjectParametersFrom( GameObject other ) {
			...
			if (animator != null) {
				Animator copiedAnimator = gameObject.AddComponent<Animator>();
				copiedAnimator.runtimeAnimatorController = animator.runtimeAnimatorController;
				copiedAnimator.updateMode = animator.updateMode;
				copiedAnimator.applyRootMotion = true;
			}
		}
