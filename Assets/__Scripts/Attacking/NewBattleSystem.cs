...
public class NewBattleSystem : MonoBehaviour
{
    ...
    public void freezeAll()
    {
        ...
        {
            Rigidbody2D r = friend.GetComponent<Rigidbody2D>();
    ...
    public void unfreezeOne(GameObject unit)
    {
        Rigidbody2D r = unit.GetComponent<Rigidbody2D>();
        r.constraints = RigidbodyConstraints2D.FreezeRotation;
    }

    public void freezeOne(GameObject unit)
    {
        Rigidbody2D r = unit.GetComponent<Rigidbody2D>();
        r.constraints = RigidbodyConstraints2D.FreezeAll;
    }
    ...
    IEnumerator EnemyTurn()
    {
        ...
        foreach (GameObject enemy in enemies)
        {
            if(enemy.TryGetComponent(out Unit u))
            {
                ...
                {
                    unfreezeOne(enemy);
                    enemy.GetComponent<MovementCircle>().SetupCircle(u.startPos, u.speed);
                    enemy.GetComponent<MovementCircle>().boundary = false;
                    ...
                    foreach (GameObject move in u.moves)
                    {
                        ...
                        // Get move score
                        float thisMoveScore = instantiatedMove.GetComponent<Move>().enemyFindOptimal(friendlies, enemy); // Calculate score for move
                        if (thisMoveScore > bestMoveScore)                                                               // If it's better than previous scores, make this the current move
                        {
                    ...
                    yield return StartCoroutine(currentMove.GetComponent<Move>().enemyAnim(enemy));                      // Use the current move (highest score)
                    enemy.GetComponent<MovementCircle>().DestroyCircle();
                }
            }
        }
